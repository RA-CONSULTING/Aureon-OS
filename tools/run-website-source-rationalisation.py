"""Isolated, package-free trust bootstrap for website source rationalisation.
...
PLANNER_PATH = Path("aureon/operator/website_source_rationalisation.py")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="run-website-source-rationalisation")
...
def main(argv: list[str] | None = None) -> int:
    ...
    launcher = _ordinary_file(Path(__file__), label="Trusted source-rationalisation launcher")
    _read_exact_source(
        launcher,
        expected_launcher,
        label="Trusted source-rationalisation launcher",
    )
    ...
    payload = _read_exact_source(
        planner,
        expected_planner,
        label="Reviewed source-rationalisation planner",
    )
