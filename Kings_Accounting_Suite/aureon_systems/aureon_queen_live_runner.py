...
THOUGHTS_FILE = Path(__file__).with_name("thoughts.jsonl")
...
_thought_bus: Optional[Any] = None
...
def get_thought_bus() -> Any:
    ...
    if _thought_bus is not None:
        return _thought_bus
    try:
        from aureon_thought_bus import get_thought_bus as canonical_bus
...
class QueenLiveRunner:
    ...
    def __init__(self, interval: float = 2.0, feed: Optional[BinancePublicMarketFeed] = None):
        if interval <= 0:
    ...
    @staticmethod
    def _unavailable_surfaces(collected_at: float) -> Iterable[tuple[str, str]]:
        return (
    ...
    def run_cycle(self) -> Dict[str, Any]:
        ...
        for topic, reason in self._unavailable_surfaces(collected_at):
            emit_telemetry(
                topic,
                {
                    "truth_status": "no_data",
