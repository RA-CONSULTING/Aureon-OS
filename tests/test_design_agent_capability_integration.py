...
def test_coding_agent_registry_snapshot_is_source_bound_and_non_authoritative() -> None:
    ...
    expected_signal_count = feedback["summary"]["signal_count"] if readiness["brief_ready"] else 0
    assert readiness["stakeholder_feedback"]["signal_count"] == expected_signal_count
...
def test_growth_planning_records_non_authoritative_registry_evidence() -> None:
    ...
    assert ingress["runner_delegation"]["shell"] is False
    assert ingress["runner_delegation"]["invoked"] is False
    assert capability.evidence["brief_ready"] is readiness["brief_ready"]
    assert capability.evidence["research_refresh_current"] is refresh["current"]
    assert (
        capability.evidence["planning_pipeline_available"]
        is readiness["planning_pipeline_available"]
    )
