...
def test_registry_discovers_current_design_roles_and_operator_capabilities() -> None:
    ...
    assert feedback["freshness"]["state"] == "current"
    ...
    assert hnc_graph["available"] is True
    ...
    assert readiness["state"] == "brief-ready"
    ...
    assert readiness["research_refresh"] == {
        "declaration_path": "data/website_operator/design_research_sources.v1.json",
        "declaration_sha256": refresh["declaration"]["sha256"],
        "state": "current",
        "passed": True,
        "artwork": {"state": "not-cleared", "cleared_for_use": False},
    }
    assert readiness["stakeholder_feedback"] == {
        "feedback_id": feedback["declaration"]["feedback_id"],
        "path": feedback["declaration"]["path"],
        "sha256": feedback["declaration"]["sha256"],
        "state": "current",
        "passed": True,
        "signal_capsules_sha256": feedback["signal_capsules_sha256"],
        "signal_count": feedback["summary"]["signal_count"],
    }
