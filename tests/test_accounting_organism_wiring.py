...
def test_accounting_context_bridge_loads_generated_artifacts_and_publishes_status():
    ...
    vault_present = REPO_ROOT.joinpath("accounting", "full_accounts_index.md").exists()
    assert context["accounting_system_registry"]["nonstandard_surfaces"]["accounting_vault_memory"] is vault_present
    assert context["accounting_vault_memory"]["status"] == ("ready" if vault_present else "missing")
