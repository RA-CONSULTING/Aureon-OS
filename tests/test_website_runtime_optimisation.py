...
def _reviewed_contract_fixture() -> tuple[dict[str, Any], dict[str, Any]]:
    """Load the tracked reviewed contract without depending on ignored run artifacts."""

    contract_path = REPO_ROOT / runtime_opt.ACCEPTANCE_CONTRACT_PATH
    contract = json.loads(contract_path.read_text(encoding="utf-8"))
    binding = contract["sourceBinding"]
    retained = {
        "manifest_sha256": binding["retainedManifestSha256"],
        "file_count": binding["retainedFileCount"],
        "total_bytes": binding["retainedTotalBytes"],
    }
