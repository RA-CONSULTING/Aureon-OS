...
def test_agent_company_has_bounded_public_website_design_studio(tmp_path: Path) -> None:
    ...
    assert all(roles[title]["brain_binding"]["registry_only"] is True for title in expected)
    assert all(
        roles[title]["brain_binding"]["canonical_brain_fabric_member"] is False
        for title in expected
    )
    assert all(roles[title]["brain_binding"]["provisioned"] is False for title in expected)
