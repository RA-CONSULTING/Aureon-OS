...
raise RuntimeError(
    "integrated_cognitive_system_unreleased_import_hold:"
...
try:
    ...
    AureonSelfFeedbackLoop = None  # type: ignore[assignment,misc]
...
try:
    from aureon.vault.ui.server import create_app as create_vault_app
    _HAS_VAULT_UI = True
except Exception:
...
class IntegratedCognitiveSystem:
    ...
    def boot(self) -> Dict[str, str]:
        """Hold the production orchestrator until Magic-Star release exists."""

        raise RuntimeError(
            "integrated_cognitive_system_boot_hold:"
            "production_magic_star_release_unavailable"
        )

    def _legacy_boot_sequence_unreachable(
        self,
        *,
        runtime_root: object,
        ...
        """
        Retained temporarily for source reconciliation; never runtime-authorized.
        """
        raise RuntimeError(
            "integrated_cognitive_system_legacy_boot_hold:"
            "production_magic_star_release_unavailable"
        )
        resolved_root = os.path.realpath(os.fspath(runtime_root))
        if (
            not test_only
            or not _background_side_effects_suppressed()
            or not os.path.isdir(resolved_root)
            or os.path.realpath(os.getcwd()) != resolved_root
        ):
            raise RuntimeError(
                "integrated_cognitive_system_test_harness_denied:"
                "explicit_isolated_test_root_required"
            )
        ...
        def boot_self_dialogue():
            if not _HAS_SELF_DIALOGUE:
                raise RuntimeError("import failed")
            raise RuntimeError("deterministic_test_harness_voice_provider_hold")
        ...
        def boot_feedback():
            if not _HAS_FEEDBACK_LOOP:
                raise RuntimeError("import failed")
            self.feedback_loop = AureonSelfFeedbackLoop(
                vault=self.vault,
                auto_wire_bus=False,
                enable_voice=False,
        ...
        def boot_sentient():
            if not _HAS_SENTIENT:
                raise RuntimeError("import failed")
            raise RuntimeError("deterministic_test_harness_ai_bridge_hold")
        ...
        def boot_vault_ui():
            if not _HAS_VAULT_UI or create_vault_app is None:
                raise RuntimeError("import failed")
            raise RuntimeError(
                "integrated_vault_ui_hold:production_magic_star_release_unavailable"
            )
    ...
    def _start_tick_thread(self) -> None:
        raise RuntimeError(
            "integrated_cognitive_tick_start_hold:"
            "production_magic_star_release_unavailable"
        )
    ...
    def _start_vault_ui(self, host: str = "127.0.0.1", port: int = 5566) -> None:
        """Hold embedded serving until the production release boundary exists."""

        _ = host, port
        raise RuntimeError(
            "integrated_vault_ui_start_hold:"
            "production_magic_star_release_unavailable"
        )
    ...
    def _start_tunnel(self, port: int) -> Optional[str]:
        """Public Vault UI tunnelling is unavailable without production release."""

        _ = port
        logger.warning(
            "Vault UI tunnel HOLD: production Plumber/Magic-Star release is unavailable"
        )
        return None
    ...
    def run(self, lan: bool = False, remote: bool = False, port: int = 5566) -> None:
        ...
        Args:
            lan: Reserved. True is rejected until a production release exists.
            remote: Reserved. True is rejected until a production release exists.
            port: Port for the vault UI / Phi Bridge server (default 5566).
        """
        if lan or remote:
            raise RuntimeError(
                "vault_ui_external_exposure_hold:production_magic_star_release_unavailable"
            )
        raise RuntimeError(
            "integrated_cognitive_system_runtime_hold:"
            "production_magic_star_release_unavailable"
        )
        ...
        if self.vault_app is not None:
            ...
            print(f"  Vault UI:    http://127.0.0.1:{port}/")
            print("  Phi Bridge:  HOLD (no LAN discovery, card gossip, or public tunnel)")
            print()
