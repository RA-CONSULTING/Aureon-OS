...
The SaaS implementation is source-visible and testable offline, but runtime and
deployment are on terminal **HOLD**. `deploy/docker-compose.saas.yml` starts only
non-networked protection preflights. The frontend image performs no package
lifecycle or source build, starts no nginx listener, and exposes no port.
Frontend `dev`, `preview`, E2E, trading, and runtime package scripts are held.

Do not inject tenant, Supabase, operator, model, or hosting credentials merely to
verify this source. No console, gateway, manifest route, tenancy service, or
payment route is deployed by the supported entrypoints.
...
Historical UI builds and smoke tests are implementation evidence only. The
current release image deliberately performs no frontend build and starts no
...
The implementation ledger below separates source work from runtime release:

| # | Phase | Item | Done? |
|---|-------|------|-------|
| 6A | Platform layer | Categorized catalog + domain adapters + honest status + gateway routes + Supabase JWT bridge. | ☑ |
| 6B | Frontend release | Historical nginx/compose design is retained in source history; active image and package routes are terminal HOLD and build/start nothing. | HOLD |
...
| 9 | Organism connectome | Source and tests cover sense/touch/weave/pulse and related APIs. Supervisor routes remain terminal HOLD; no dormant organ or HNC daemon is started. | SOURCE ONLY |
